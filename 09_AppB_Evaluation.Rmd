...
#Specify rows of the table
TableB.2[1,] <- c("Freshwater Tidal (fresh.tid)", "New Jersey Land Use", "tidal freshwater marshes (6120)")
TableB.2[2,] <- c("Freshwater Tidal (fresh.tid)", "National Wetland Inventory", "Riverine, tidal systems with emergent vegetation (R1EM), unconsolidated bottom (R1UB), and rock bottom (R1RB)")
TableB.2[3,] <- c("Freshwater Tidal (fresh.tid)", "National Wetland Inventory", "Only riverine, tidal systems with emergent vegetation (R1EM)")
TableB.2[4,] <- c("Estuarine Intertidal (est.int)", "New Jersey Land Use", "saline marsh (6110) and tidal mud flat (5412)")
TableB.2[5,] <- c("Estuarine Intertidal (est.int)", "National Wetland Inventory", "All estuarine, intertidal systems (E2)")
TableB.2[6,] <- c("Estuarine Intertidal (est.int)", "National Wetland Inventory", "Only estuarine, intertidal emergent systems (E2EM)")
TableB.2[7,] <- c("Estuarine Subtidal - Hard Bottom (est.sub.hard)", "National Wetland Inventory", "Estuarine, subtidal, rock bottom (E1RB) and estuarine, subtidal reef (E1RF)")
TableB.2[8,] <- c("Estuarine Subtidal - Soft Bottom - SAV (est.sub.soft.SAV)", "National Wetland Inventory", "Estuarine, subtidal, aquatic bed (E1AB)")
TableB.2[9,] <- c("Estuarine Subtidal - Soft Bottom - clam (est.sub.soft.clam)", "National Wetland Inventory", "Estuarine, subtidal, unconsolidated bed (E1UB)")
...
The freshwater tidal sub-model was verified against the State of New Jersey's land use classes and the National Wetland Inventory. Table B.3 presents results of the efficacy of model classification for the New Jersey Back Bays system. **These analyses indicate...**

```{r, echo=FALSE}
#Load the confusion matrix from the verification analysis
load("confusion.Rdata")

#Isolate the freshwater tidal rows
#TableB3 <- confusion[1:3,2:6]
#rownames(TableB3) <- NULL

#Send output table rows into a single matrix
#knitr::kable(TableB3, caption="NYBEM classification efficacy summarized via a confusion matrix.", align="c") 
```


The freshwater tidal habitat suitability index was examined relative to the presence and absence of the New Jersey wetland data set (Figure B.1). This analysis indicates that NYBEM strongly differentiates habitat quality in patches with and without existing wetlands. Specifically, the mean HSI value for patches with existing wetlands is 0.505 with an interquartile range (IQR) of 0.501-0.553, whereas the mean HSI value for patches without existing wetlands is 0.266 with an interquartile range (IQR) of 0.203-0.240. 

```{r, echo=FALSE, fig.cap="NYBEM fresh.tid verification relative to the presence and absence of observed wetland habitats."}
knitr::include_graphics("ZZ_Fig04.01_NYBEM.Architecture.png")
```


The final verification analysis examines the aggregate predictions of habitat extent across large-scales. Specifically, the extent of habitat predicted by NYBEM relative to observed habitat on an inlet-by-inlet basis. Figure B.2A shows NYBEM predictions of habitat area only, which over-predicts habitat extent as anticipated. Figure B.2B shows habitat quantity and quality via the combined habitat unit, which produces high predictive accuracy. Notably, all areas are converted to acres for consistency with standard presentation of habitat units. **Based on the three analyses presented, the NYBEM's outcomes are deemed useful for...**
...
The estuatine intertidal sub-model was verified against the State of New Jersey's land use classes and the National Wetland Inventory. Table B.3 presents results of the efficacy of model classification for the New Jersey Back Bays system. **These analyses indicate...**

```{r, echo=FALSE}
#Isolate the estuarine intertidal rows
#TableB3 <- confusion[1:3,2:6]
#rownames(TableB4) <- NULL

#Send output table rows into a single matrix
#knitr::kable(TableB4, caption="NYBEM classification efficacy for the est.int submodel summarized via a confusion matrix.", align="c") 
```

The freshwater tidal habitat suitability index was examined relative to the presence and absence of the New Jersey wetland data set (Figure B.3). This analysis indicates that NYBEM strongly differentiates habitat quality in patches with and without existing wetlands. Specifically, the mean HSI value for patches with existing wetlands is 0.505 with an interquartile range (IQR) of 0.501-0.553, whereas the mean HSI value for patches without existing wetlands is 0.266 with an interquartile range (IQR) of 0.203-0.240. 

```{r, echo=FALSE, fig.cap="NYBEM fresh.tid verification relative to the extent of habitat predicted across eleven inlets."}
knitr::include_graphics("ZZ_FigB03_est.int.boxplot.jpeg")
```

The final verification analysis examines the aggregate predictions of habitat extent across large-scales. Specifically, the extent of habitat predicted by NYBEM relative to observed habitat on an inlet-by-inlet basis. Figure B.4A shows NYBEM predictions of habitat area only, which over-predicts habitat extent as anticipated. Figure B.4B shows habitat quantity and quality via the combined habitat unit, which produces high predictive accuracy. Notably, all areas are converted to acres for consistency with standard presentation of habitat units. **Based on the three analyses presented, the NYBEM's outcomes are deemed useful for...**
