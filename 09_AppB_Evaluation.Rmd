...
#Specify rows of the table
TableB.2[1,] <- c("Freshwater Tidal (fresh.tid)", "National Wetland Inventory", "Only riverine, tidal systems (R1)")
...
TableB.2[4,] <- c("Estuarine Subtidal - Hard Bottom (est.sub.hard)", "National Wetland Inventory", "Estuarine, subtidal, rock bottom (E1RB) and estuarine, subtidal reef (E1RF)")
TableB.2[5,] <- c("Estuarine Subtidal - Soft Bottom - SAV (est.sub.soft.SAV)", "National Wetland Inventory", "Estuarine, subtidal, aquatic bed (E1AB)")
TableB.2[6,] <- c("Estuarine Subtidal - Soft Bottom - clam (est.sub.soft.clam)", "National Wetland Inventory", "Estuarine, subtidal, unconsolidated bed (E1UB)")
...
The freshwater tidal sub-model was verified against National Wetland Inventory data, specifically the the riverine tidal class (code: R1). Presence-absence outcomes for each cell in the NJBB domain were used to assess the relative rate of classification error. True positive, false positive, false negative, and true negative rates were 0.0%, 9.70%, 0.01%, and 90.29%, respectively. These analyses indicate high rates of false positivity and dramatic overprediction of the extent of freshwater tidal habitat.


The freshwater tidal habitat suitability index was examined relative to the presence and absence of the New Jersey wetland data set (Figure B.1). This analysis indicates that NYBEM weakly differentiates habitat quality in patches with and without existing wetlands. Specifically, the mean HSI value for patches with existing wetlands is 0.040 (n=1,555), whereas the mean HSI value for patches without existing wetlands is 0.042 (n=11,705,193). 

```{r, echo=FALSE, fig.cap="NYBEM fresh.tid verification relative HSI outcomes in observed NWI patches."}
knitr::include_graphics("ZZ_FigB01_freshwater.violin.jpeg")
```


The final verification analysis examines the aggregate predictions of habitat extent across large-scales. Specifically, the extent of habitat predicted by NYBEM relative to observed habitat on an inlet-by-inlet basis. Figure B.2A shows NYBEM predictions of habitat area only, and Figure B.2B shows habitat quantity and quality via the combined habitat unit. Both estimates dramatically over-predicts habitat extent by an order-of-magnitude.
...
Based on these outcomes, the fresh.tid model significantly over-predicts ecosystem extent relative to both binary (i.e., presence-absence) and continuous (i.e., HSI) outcomes. As such, the model should not be used to numerically estimate the extent or quality freshwater ecosystems. However, the model does strongly differentiate estimates of habitat extent and quality by inlet (Figure B.2), which could indicate that relative comparison between systems may be appropriate.
...
Note about the input data sets. Presence-absence data and confusion analysis.
