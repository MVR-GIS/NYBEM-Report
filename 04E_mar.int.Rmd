...
* USFWS Habitat Evaluation Procedure models: tern (@Carreker_1985, @USFWS_2001), plover [@USFWS_2001], horseshoe crab [@brady_1996],[@USFWS_2001], red knot [@USFWS_2001]

* Other habitat suitability models: plover (@farmer_2000, @Seavey_2011), horseshoe crab [@avissar_2006], [@jackson_2010], [Lathrop_2013], amaranth [@sellars_2007], sea turtles [@dunkin_2016]  
* Fire Island to Montauk Point Habitat Evaluation Procedure OceanBeach Sub-Model [@USACE_2009].  
* Empirical studies of the system: Virginia Tech shorebird program (Hermanns et al. 2018), Rutgers horseshoe crab mapping [@lathrop_2013], international shorebird survey (Manomet)
...
Beach morphology directly affects habitat use by multiple focal taxa (e.g., shorebirds, turtles, horseshoe crab). Beach shape and morphologic character could be described through parameters of width, slope, elevation, and other summaries of topography [@bridges_2015]. Beach width is indirectly captured in NYBEM through metrics of habitat quantity and patch size and is not considered here. Beach slope integrates morphologic outcomes and is directly tied to grain size and wave environment [@lodder_2021]. Beach slope, thus, provides an important and synthetic metric of marine, intertidal habitat quality.


More specifically, beach slope has been identified as an important driving component of habitat use by multiple taxa in the New York Bight. Seabeach amaranth (*Amaranthus pumilus*) typically occurs in low slope environments [@sellars_2007]. Habitat is most suitable for horseshoe crabs between ~5-10% slopes [@brady_1996]. Slope has been used as a general indicator of ecosystem condition for Long Island ecosystems (FIMP HEP 2009). Notably, not all taxa may respond in kind to these generalization; for instance, sea turtles have shown habitat preferences for moderate slopes between ~10-30% in subtropical areas such as Southeastern Florida. Mereging these regional models, NYBEM assesses suitability relative to beach slope as follows:
...
Intertidal zones are a key foraging habitat for shorebirds. The benthic food availability is related in part to soil moisture [@brady_1996] and the time to forage in these zones. Soil moisture is also a key determinant of horseshoe crab spawning [@avissar_2006]. For NYBEM, a proxy variable is used to account for these processes. Specifically, a ratio of depth ranges to the tidal range is used as a relative duration of exposure, which we calculate as follows:
...
Marine, intertidal zones are a key transitional habitat between aquatic and upland systems. Fully functional intertidal areas would include high-quality adjacent uplands and subtidal areas. A variety of metrics have been used to assess human impacts on this system, including diverse metrics like foot traffic, vehicle use of beaches, beach grooming and maintenance practices, physical barriers along the beach [@farmer_2000], direct human development such as buildings, trash and debris presence (USACE FIMP 2009), and general levels of land use intensity [@Seavey_2011].
...
Vegetative cover in the intertidal zone produces both positive and negative ecological effects. Animal movement can be disrupted by the presence of vegetation, such as the precipitous declines in habitat suitability for least tern beyond 20-25% herbceous or shrub canopy cover [@carreker_1985]. Conversely, vegetation can be a positive attribute and a key transitional feature moving upland toward dune environments, and the difference between the lower intertidal (below MTL) and higher intertidal (above MTL) can be pronounced (FIMP HEP 2009). **REVISIT THIS SUITABILITY CURVE BASED ON GULF OF MAINE MODELS.** 
...
* Substrate is a key driver of beach outcomes in many contexts [@lodder_2021], and specifically, substrate is often included in regional models of this system (e.g., @brady_1996, @avissar_2006). However, high quality substrate data were unavilable, which created a major data gap for incorporating this variable.  
* Other assessment procedures (@farmer_2000, @USACE_2009) have included a variety of human development pressures related to local features (e.g., vehicle and foot traffic), which could provide important refinement of the human use intensity metrics for land use and shoreline development.  
